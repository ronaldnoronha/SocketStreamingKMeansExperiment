    ...
    def start(self):
        ...
        while not self.__terminate:
            try:
                ...
                disk_io_counters = ','.join([str(i) for i in psutil.disk_io_counters()])
                network_io_lo = ','.join([str(i) for i in psutil.net_io_counters(pernic=True)['lo']])
                network_io_ens3 = ','.join([str(i) for i in psutil.net_io_counters(pernic=True)['ens3']])
            finally:
                memory_percent: float = psutil.virtual_memory().percent
                self.__logs.append(str(datetime.datetime.now().time()) + "," + str(cpu_percent) + "," +
                                   str(memory_percent) + "," + disk_io_counters + "," + network_io_lo + "," +
                                   network_io_ens3 + "\n")
        self._write_logs()
